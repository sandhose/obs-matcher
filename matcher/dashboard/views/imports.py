...
class ImportFileListView(View, DbMixin):
    def dispatch_request(self):
        ...
        if form.validate_on_submit():
            ...
            last_import = self.session.query(ImportFile).\
            ...
            for attr in ['imported_external_object_type', 'platform_id', 'fields']:
                setattr(file, attr, getattr(last_import, attr, getattr(file, attr, None)))
        ...
        if form.validate_on_submit():
            if file.status != ImportFileStatus.UPLOADED:
                ...
                if form.save_and_import.data:
                    self.celery.send_task('matcher.tasks.import_.process_file', [file.id])
