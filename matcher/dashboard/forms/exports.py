from wtforms import (Form, FormField, SelectMultipleField, StringField,
                     validators,)
...
class ExportFilter(Form):
    platform_id = QuerySelectMultipleField("Platform", get_label='name', render_kw={'class': 'select2'})
    platform_group_id = QuerySelectMultipleField("Group", get_label='name', render_kw={'class': 'select2'})
    platform_country = SelectMultipleField("Platform country", render_kw={'class': 'select2'})
    platform_type = SelectMultipleField("Platform type", choices=[(v.name, v.name) for v in PlatformType],
                                        render_kw={'class': 'select2'})

    def render(self):
        filters = {}
        for i in ['platform_id', 'platform_group_id', 'platform_country', 'platform_type']:
            attr = getattr(self, i)
            if attr.data:
