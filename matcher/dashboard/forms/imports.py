...
class Column(Form):
    key = StringField(
        "Column name", render_kw={"class": "form-control", "disabled": "disabled"}
    )
    ...
    arg_platform = SelectField(
        "Platform",
        render_kw={"class": "select2 form-control dynamic-input", "data-show": "link"},
    )

    arg_attribute = SelectField(
        "Attribute",
        choices=[(str(e), badge_display(e)) for e in ValueType],
        render_kw={
            "class": "select2 form-control dynamic-input",
...
class CustomFieldList(FieldList):
    ...
    def populate_obj(self, obj, name):
        ...
        for entry in self.entries:
            ...
            value = (
                type_ if arg is None else "{type_}.{arg}".format(type_=type_, arg=arg)
            )
...
class EditImport(FlaskForm):
    ...
    save_and_import = SubmitField(
        "Save and process", render_kw={"class": "btn btn-primary"}
    )
