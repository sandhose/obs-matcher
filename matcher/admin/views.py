...
def links_formatter(route):
    def formatter(view, context, model, name):
        list = "".join(
            [
                '<li><a href="{}">{}</a></li>'.format(url_for(route), escape(item))
                for item in getattr(model, name)
            ]
        )
        return rules.Markup("<ul>" + list + "</ul>")
...
def link_formatter(route):
    def formatter(view, context, model, name):
        return rules.Markup(
            '<a href="{}">{}</a>'.format(
                url_for(route, id=getattr(model, name).id), escape(getattr(model, name))
            )
        )
...
def series_formatter(view, context, model, name):
    ...
    if series is not None:
        return rules.Markup(
            '<a href="{}">{}</a>'.format(
                url_for("series.details_view", id=series.id), escape(repr(series))
            )
        )
...
def episodes_formatter(view, context, model, name):
    ...
    return rules.Markup(
        '<a href="{}">{}</a>'.format(
            url_for("episodes.index_view", flt1_5=model.id), count
        )
    )


def attribute_formatter(
    f=lambda _: True, show_score=False, filter=lambda _: True, limit=None
):
    def formatter(view, context, model, name):
        attrs = [attr for attr in model.values if f(attr.type) and filter(attr.text)]
...
class PlatformGroupView(DefaultView):
    ...
    column_formatters = {"platforms": count_formatter}
...
class ValueView(DefaultView):
    ...
    column_details_list = ("id", "external_object", "type", "text", "score", "sources")
...
class ExternalObjectView(DefaultView):
    def __init__(self, *args, **kwargs):
        ...
        super(ExternalObjectView, self).__init__(ExternalObject, *args, **kwargs)
    ...
    column_formatters = {
        ...
        "date": attribute_formatter(
            partial(eq, ValueType.DATE), filter=lambda t: len(t) == 4
        ),
        "genres": attribute_formatter(partial(eq, ValueType.GENRES), limit=3),
        "country": attribute_formatter(
            partial(eq, ValueType.COUNTRY), filter=lambda t: len(t) == 2
        ),
        "duration": attribute_formatter(
    ...
    column_extra_row_actions = [
        EndpointLinkRowAction(
            "glyphicon icon-search", "allobjects.index_view", id_arg="flt0_0"
        )
    ]
    ...
    column_filters = [
        ...
        ExternalObjectPlatformFilter(
            ...
            name="Platform",
            options=[
                (p.slug, p.name) for p in db.session.query(Platform.slug, Platform.name)
            ],
        ),
        ExternalObjectPlatformFilter(
            column=Platform.slug,
    ...
    def get_query(self):
        q = (
            super(ExternalObjectView, self)
            .get_query()
            .options(joinedload(ExternalObject.values).joinedload(Value.sources))
        )
...
class SeriesView(ExternalObjectView):
    ...
    column_details_list = ("id", "type", "values_list", "links_list", "episodes_list")
    column_list = (
        "id",
        "title",
        "date",
...
class EpisodeView(ExternalObjectView):
    ...
    column_details_list = ("id", "type", "values_list", "links_list", "episode_details")
    column_filters = ExternalObjectView.column_filters + [SeriesFilter(name="Episode")]
