...
def upgrade():
    ...
    op.create_table(
        'export_template',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('row_type', sa.Enum('EXTERNAL_OBJECT', 'OBJECT_LINK', name='exportrowtype'), nullable=False),
        sa.Column('external_object_type',
                  postgresql.ENUM('PERSON', 'MOVIE', 'EPISODE', 'SERIES', name='externalobjecttype', create_type=False),
                  nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('iterator', sa.Enum('PLATFORMS', 'GROUPS', 'COUNTRIES', name='exportfilteriterator'), nullable=True),
        sa.Column('file_path_template', sa.String(), nullable=False),
        sa.Column('filters_template', postgresql.HSTORE(text_type=sa.Text()), nullable=False),
        sa.ForeignKeyConstraint(['export_template_id'], ['export_template.id'],
    ...
    op.create_table(
        'export_file',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('status', sa.Enum('PROCESSING', 'DONE', name='exportfilestatus'),
                  server_default='PROCESSING', nullable=True),
        ...
        sa.ForeignKeyConstraint(['export_factory_id'], ['export_factory.id'],
                                name='fk_export_file_export_factory_id_export_factory'),
        sa.ForeignKeyConstraint(['export_template_id'], ['export_template.id'],
                                name='fk_export_file_export_template_id_export_template'),
        sa.ForeignKeyConstraint(['session_id'], ['session.id'], name='fk_export_file_session_id_session'),
