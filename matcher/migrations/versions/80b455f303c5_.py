...
primary_keys = ['episode', 'external_object', 'object_link', 'platform_group', 'platform', 'role', 'scrap_link',
                'scrap', 'value', 'value_source', 'vw_attributes', 'vw_value_score']
...
def upgrade():
    ...
    for table, column, foreign_table in foreign_keys:
        op.execute("ALTER TABLE {table} RENAME CONSTRAINT {table}_{column}_fkey TO fk_{table}_{column}_{foreign_table}"
                   .format(table=table, column=column, foreign_table=foreign_table))
...
def downgrade():
    ...
    for table, column, foreign_table in foreign_keys:
        op.execute("ALTER TABLE {table} RENAME CONSTRAINT fk_{table}_{column}_{foreign_table} TO {table}_{column}_fkey"
                   .format(table=table, column=column, foreign_table=foreign_table))
