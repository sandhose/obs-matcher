...
def upgrade():
    ...
    op.create_foreign_key('episode_serie_id_fkey', 'episode', 'external_object', ['serie_id'], ['id'])
    op.drop_column('episode', 'season_id')
    op.drop_column('episode', 'number')


def downgrade():
    op.add_column('episode', sa.Column('number', sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column('episode', sa.Column('season_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint('episode_serie_id_fkey', 'episode', type_='foreignkey')
    op.create_foreign_key('episode_season_id_fkey', 'episode', 'external_object', ['season_id'], ['id'])
    op.drop_column('episode', 'serie_id')
    op.drop_column('episode', 'season')
    op.drop_column('episode', 'episode')
    op.create_table('season',
                    sa.Column('external_object_id', sa.INTEGER(), autoincrement=False, nullable=False),
