...
def test_list(client, session):
    ...
    response = client.get("/api/groups/")
    assert response.json["items"] == [
        {"id": group.id, "name": group.name, "platforms": []}
    ]
...
def test_get(client, session):
    ...
    assert response.json == {"id": group.id, "name": group.name, "platforms": []}

    platform = Platform(
        name="bar", slug="bar", type=PlatformType.INFO, country="US", group=group
    )
    session.add(group)
    session.commit()
